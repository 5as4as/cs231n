...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out,hidden_cache = affine_forward(hidden,W2,b2)
        scores = out
        ...
        dhidden,dW2,db2 = affine_backward(dscore,hidden_cache)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization == 'batchnorm' or self.normalization == 'layernorm':
                ...
                if self.normalization == 'batchnorm':
                    output,cache = affine_bn_relu_forward(inputs, w, b, gamma, beta, self.bn_params[i])
                else:
                    output,cache = affine_ln_relu_forward(inputs, w, b, gamma, beta, self.bn_params[i])
            else:
                output,cache = affine_relu_forward(inputs, w, b)
            ...
            inputs = output
            caches[i+1]=cache
        output,affine_cache = affine_forward(inputs,self.params[f'W{self.num_layers}'],self.params[f'b{self.num_layers}'])
        scores = output
        ...
        reg = self.reg
        loss,dscore = softmax_loss(scores,y)
        loss += sum([0.5*reg*np.sum(self.params[f'W{i+1}']*self.params[f'W{i+1}']) for i in range(self.num_layers)])
        dhidden,dw,db = affine_backward(dscore,affine_cache)
        dw += reg*self.params[f'W{self.num_layers}']
